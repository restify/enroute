            ...
            for (const [routeName, methods]
                ...
                for (const [method, src] of Object.entries(methods)) {
                    ...
                    try {
                        ...
                        var func = mod && mod.__esModule && 'default' in mod
                            ? mod.default
                            : mod;
...
function reloadProxy(opts, cb) {
    // TODO: support hot reloading for require(esm)
    // the challenge here is that node doesn't provide a mechanism to clear
    // the import cache, see https://github.com/nodejs/node/issues/49442
    assert.object(opts, 'opts');
    assert.string(opts.basePath, 'opts.basePath');
    assert.optionalString(opts.excludePath, 'opts.excludePath');
